...
namespace {
constexpr uint8_t XBOX_CONTROLLER_INDEX_BUTTONS_DIR = 12;
constexpr uint8_t XBOX_CONTROLLER_INDEX_BUTTONS_MAIN = 13;
constexpr uint8_t XBOX_CONTROLLER_INDEX_BUTTONS_CENTER = 14;
constexpr uint8_t XBOX_CONTROLLER_INDEX_BUTTONS_SHARE = 15;
}  // namespace
...
uint8_t XboxControllerNotificationParser::update(uint8_t* data, size_t length) {
  ...
  joyLHori = static_cast<uint16_t>(data[0]) | (static_cast<uint16_t>(data[1]) << 8);
  joyLVert = static_cast<uint16_t>(data[2]) | (static_cast<uint16_t>(data[3]) << 8);
  joyRHori = static_cast<uint16_t>(data[4]) | (static_cast<uint16_t>(data[5]) << 8);
  joyRVert = static_cast<uint16_t>(data[6]) | (static_cast<uint16_t>(data[7]) << 8);
  trigLT = static_cast<uint16_t>(data[8]) | (static_cast<uint16_t>(data[9]) << 8);
  trigRT = static_cast<uint16_t>(data[10]) | (static_cast<uint16_t>(data[11]) << 8);
  return 0;
}
